classdef PIDControl < handle
    ...
    methods
        %----------------------------
        function self = PIDControl(kp, kd, ki, limit, beta, Ts)
        ...
        function u = PID(self, y_r, y, flag)
            ...
            % PD Control
            if flag==true
                u_unsat = self.kp*error + self.kd*self.error_dot + self.ki*self.integrator;
            else
                u_unsat = self.kp*error - self.kd*self.y_dot + self.ki*self.integrator;
            end
        ...
        %----------------------------
        function self = integratorAntiWindup(self, error)%u_sat, u_unsat)
            if self.ki~=0
